...
pub fn run(old_commit: &str, new_commit: &str) -> Result<()> {
    ...
    let old_members = members_at_commit(&repo, old_commit).context("get crate members")?;
    let new_members = members_at_commit(&repo, new_commit).context("get crate members")?;
...
/// Reads a workspace's Cargo package versions as they existed at `commit`, by checking it out
/// into a throwaway git worktree without disturbing the caller's current checkout — the one
/// piece of `tag`'s logic that actually needs git; `compute_tags`/`tag` (the actual
/// version-diffing rules) stay git-agnostic and take plain `&[Package]`.
fn members_at_commit(repo: &Repository, commit: &str) -> Result<Vec<Package>> {
    let oid = repo
