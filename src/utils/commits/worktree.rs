...
impl PackageCommits for WorktreeCommitAssigner {
    ...
    fn get(
        ...
        let changelog_range = config.commit_range();
        ...
        let attributed = {
            ...
            let changed =
                get_changed_packages(&repo, config, packages).context("get changed packages")?;

            attribute_commits_to_packages(&repo, &all_commits, changed)
...
pub fn fetch_remote(repo: &Repository, config: &Config) -> Result<()> {
...
pub fn get_commits<'a>(repo: &'a Repository, release: &ReleaseConfig) -> Result<Vec<Commit<'a>>> {
...
// Determines which packages actually differ between HEAD and the merge-base with
// `<remote>/<default_branch>` — i.e. what's really different from the current upstream state,
// regardless of how the branch's history got there (merges, reverts, rebases, ...). This is the
// authoritative source for which packages need a version bump.
//
