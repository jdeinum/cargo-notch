...
impl PackageItem {
    /// The patch/minor/major options are computed from `package`'s current version. `suggested`
    /// — the package's state machine's suggested bump, from `NotchFsm::suggested_bump` — is
    /// preselected, letting the user confirm it as-is or override it before confirming.
    pub fn new(package: Package, commits: Vec<CommitInfo>, suggested: Option<Bump>) -> Self {
        let current = package.version.clone();
        let patch = Bump::Patch.apply_to(&current);
        let minor = Bump::Minor.apply_to(&current);
        let major = Bump::Major.apply_to(&current);
        Self {
            package,
            commits,
            patch,
...
/// Runs the interactive bump-selection TUI over `fsms`, one per changed package. Each package's
/// row is preselected with its state machine's suggested bump (see `PackageItem::new`). Returns
/// `None` if the user cancelled (nothing should be written to disk), or the confirmed selections
/// otherwise.
pub fn run(fsms: HashMap<Package, NotchFsm>) -> Result<Option<Vec<UpdatedCrate>>> {
    let mut packages: Vec<PackageItem> = fsms
        .into_iter()
        .map(|(package, fsm)| {
            let suggested = fsm.suggested_bump();
            let commits = fsm.commits().to_vec();
            PackageItem::new(package, commits, suggested)
        })
        .collect();
...
mod tests {
    ...
    fn bump_options_are_computed_from_the_current_version() {
        ...
        assert_eq!(
            *item.version_for(Bump::Patch),
            Version::parse("1.2.6").unwrap()
        );
        assert_eq!(
            *item.version_for(Bump::Minor),
    ...
    #[test]
    fn a_package_with_a_suggested_bump_starts_preselected() {
        ...
        assert_eq!(item.selected, Some(Bump::Minor));
    }

    #[test]
    fn a_package_with_no_suggested_bump_starts_unselected() {
